...
void mmc5893ma_init(rt_sensor_t sensor)
{
    ...
    rt_uint8_t send_buf[]={MMC5983MA_CR0_ADDR,cr0.r};//开启测量完成中断，开启自动sr，中断只是用来亮灯
...
rt_err_t mmc5893ma_polling_get_mag(void)
{
    ...
    rt_uint8_t send_buf[]={0x80|0x22};
    rt_uint8_t recv_buf[7]={0};

    result=rt_spi_send_then_recv(spi_dev,send_buf,sizeof(send_buf),recv_buf,2);

    // rt_int32_t x=(recv_buf[0]&0x80>0)?:;
